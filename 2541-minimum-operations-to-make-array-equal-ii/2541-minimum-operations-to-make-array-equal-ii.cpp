...
public:
    long long minOperations(vector<int>& nums1, vector<int>& nums2, int k) 
    {
        ...
        {
            if(nums1[i] >= nums2[i]) p_diff += nums1[i]-nums2[i];
            else n_diff += nums2[i]-nums1[i];
        }
        
        long long ans=-1;
        //if both difference(p_diff,n_diff) are equal, we can make nums1 equals to nums2
        //we also need to check whether we can make them equal by using k
        if((p_diff-n_diff==0) && (p_diff%k)==0)
        {
             ans = (p_diff/k);
        }
        return ans;
